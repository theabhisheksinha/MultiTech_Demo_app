...
FORM branche_contratcompte .
  ...
  CHECK NOT s_title_id[] IS INITIAL.

* Lecture contrat compte
  SELECT  * INTO TABLE gt_titleauthor FROM titleauthor
                          WHERE title_id IN s_title_id
                          AND   royaltyper IN rt_royaltyper. "vide



  IF sy-subrc <> 0.
    MESSAGE i004 WITH 'Compte de contrat'(004).
    LEAVE PROGRAM.
  ENDIF.
...
* title/au_id limité
      ELSEIF NOT s_title IS INITIAL AND NOT s_au_id IS INITIAL.
        MESSAGE e029.
* title_id/au_id limité
      ELSEIF NOT s_title_id IS INITIAL AND NOT s_au_id IS INITIAL.
        MESSAGE e029.
* Pas de réponse -id
      ELSEIF NOT s_title IS INITIAL AND p_auid IS INITIAL.
        MESSAGE e025.
...
*  e_printparams-tddest = p_imprime.
*
*
*  CALL FUNCTION 'EFG_GET_PRINT_PARAMETERS'
*   EXPORTING
*     x_printparams                 = e_printparams
...
*
** SAPscript Ausgabe Schnittstelle
*  MOVE-CORRESPONDING e_printparams TO p_itcpo.
** Zusätzliche Druckparameter zur ITCPO
*  MOVE-CORRESPONDING e_printparams TO p_efgpp.
*
